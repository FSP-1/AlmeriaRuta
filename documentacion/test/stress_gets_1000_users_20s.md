...
## Recomendaciones prácticas para mejorar (si quieres)

- **No usar el servidor de desarrollo de Flask en estrés real**: levantar la API con un servidor WSGI (gunicorn/waitress) con múltiples workers.
- **Cachear respuestas estáticas** (`/lines` y `/lines/<id>/stops`) en memoria para evitar recomputar.
- **Optimizar `/arrivals`** para que sea O(n) simple y sin lecturas pesadas.
- **Repetir pruebas** con mixes separados:
  - Solo `/arrivals`: `--mix 0,0,10,0`
  - Solo `/lines`: `--mix 10,0,0,0`

### Mix más realista (con caché en cliente)

Si quieres simular mejor el comportamiento de usuarios con caché activada (stops/líneas ya descargadas), usa un mix centrado en arrivals:

- Principalmente `/arrivals` (lo más parecido a “usuarios mirando tiempos”):
  - `python stress_gets.py --duration 20 --concurrency 1000 --timeout 6 --mix 0,1,9,0`
  - o incluso `--mix 0,0,10,0`

Esto te dará una visión más fiel de la estabilidad/latencia del endpoint que más se consulta durante el uso normal.
